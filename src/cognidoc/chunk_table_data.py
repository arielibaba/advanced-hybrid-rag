...
def chunk_markdown_table(
    ...
    Args:
        ...
        - n_tokens (int): Maximum number of tokens per chunk (including header tokens).
        - overlap (int): Number of tokens to overlap between adjacent chunks (data‐rows only).
        - ollama_client (ollama.Client, optional): The Ollama client to use for querying the model.
        - model (str): The name of the model to use for chunking (Ollama mode only).
        - model_options (Dict, optional): Options for the model, such as temperature (Ollama mode only).
        - use_unified_llm (bool): If True, use the unified LLM client (Gemini by default) instead of Ollama.
        - temperature (float): Temperature for generation when using unified LLM (default 0.7).
    ...
    if use_unified_llm:
        output = ask_llm_json_unified(formatted_prompt, temperature=temperature)
    else:
        output = ask_LLM_with_JSON(formatted_prompt, ollama_client, model, model_options or {})
...
def chunk_table_data(
    ...
    Args:
        ...
        - n_tokens (int): The chunk size.
        - overlap (int): Number of tokens to overlap between adjacent chunks (data‐rows only).
        - tables_chunks_dir (str): The path to the directory where to store the generated tables chunks.
        - ollama_client (Ollama Client, optional): The Ollama client to use (legacy mode only).
        - model (str): The name of the model to use for chunking (legacy Ollama mode only).
        - model_options (Dict, optional): Options for the model (legacy Ollama mode only).
        - use_unified_llm (bool): If True (default), use unified LLM client (Gemini). If False, use Ollama.
        - temperature (float): Temperature for generation when using unified LLM (default 0.7).
            ...
            if table_path.is_file():
                ...
                with open(table_path, "r", encoding="utf-8") as f:
                    ...
                    chunks, _, summary = chunk_markdown_table(
                        prompt, md_table, None, n_tokens, overlap,
                        ollama_client=ollama_client, model=model, model_options=model_options,
                        use_unified_llm=use_unified_llm, temperature=temperature
                    )
        ...
        for idx, table_path in enumerate(tables_path.rglob("*.md")):
            if table_path.is_file():
                ...
                with open(table_path, "r", encoding="utf-8") as f:
                    md_table = f.read()
                    chunks, _, summary = chunk_markdown_table(
                        prompt, md_table, cols[idx], n_tokens, overlap,
                        ollama_client=ollama_client, model=model, model_options=model_options,
                        use_unified_llm=use_unified_llm, temperature=temperature
                    )
