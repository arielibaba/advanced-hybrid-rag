...
def parse_image_with_table_func(image_path, model, processor, config, prompt, output_dir):
...
def parse_image_with_table(image_dir, model_dir, prompt, output_dir):
    ...
    # Process image files with tables only in the directory
    # Extract the tables

    print(f"\nProcessing the images with tables only in directory: {image_dir} ...\n")

    for image_path in Path(image_dir).glob('*_Table_*.jpg'):
        parse_image_with_table_func(image_path, model, processor, config, prompt, output_dir)
    
    print(f"\nAll the images were processed successfully!\nThe tables extracted were saved to: {output_dir}\n")
