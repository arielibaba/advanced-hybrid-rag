...
- **~~Aggregate/exhaustive queries~~** *(partially addressed)*: `exhaustive_search` now searches the entire BM25 index and returns total match count, source document list, and excerpts. A coverage warning is included when the response is based on a sample. `retrieve_vector` and `exhaustive_search` now accept `source_filter` to target specific documents.
...
**Aggregate and exhaustive queries** *(partially addressed)*: The `exhaustive_search` agent tool now searches the entire BM25 index (all documents, not top-K) and returns total match count, source document list, and excerpts. A coverage warning is automatically included in the observation when the response is based on a sample (e.g., "Showing 5 of 342 matches"). Both `retrieve_vector` and `exhaustive_search` accept `source_filter` to target specific documents. Remaining gap: the knowledge graph's entity extraction coverage depends on LLM quality, so entity-level aggregation ("how many entities of type X?") may still be incomplete.

**Temporal and conditional queries** *(partially addressed)*: The `source_filter` parameter on `retrieve_vector` and `exhaustive_search` lets the agent target specific documents by name. For queries like "what does report_2024 say about X?", the agent can scope retrieval to that document. Remaining gap: date-based filtering requires date metadata in chunks (not yet extracted at ingestion). For implicit temporal information spread across passages, the agent's multi-step retrieval helps but cannot guarantee complete cross-referencing.
